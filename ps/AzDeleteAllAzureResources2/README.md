...
4. 各リソースグループについて以下を実施:
   - **リソースロックの確認**（ロックがある場合は ❌ マークを表示してスキップ）
   - ロックがない場合のみ、リソースごとの削除順序に従ってリソース削除
...
- ⚠️ **リソースロック付きリソースグループの扱い**
  - ロックがかかっているリソースグループは**自動的にスキップ**されます
  - ロックの自動解除機能はありません
  - ロック付きリソースグループも削除したい場合は、**手動でロックを解除**してから再実行してください
  - または、**Version 3（AzDeleteAllAzureResources3）を使用**すると、ロック削除を選択できます
...
```plaintext
Checking resource group: rg-test ...
❌ Lock detected on resource group 'rg-test'. Skipping deletion.

Checking resource group: rg-demo ...
✅ No lock detected. Proceeding with resource deletion in 'rg-demo'...
```

ロックがあるリソースグループは赤い ❌ マークでスキップされます。

## よくある質問 (FAQ)

| 質問                                               | 回答                                                       |
| :------------------------------------------------- | :--------------------------------------------------------- |
| Public IP が Firewall に紐づいていて削除できない！ | 自動で Firewall の IP 構成から解除 → 更新します            |
| Firewall 更新に失敗したら？                        | リトライ処理やスキップ処理により続行します                 |
